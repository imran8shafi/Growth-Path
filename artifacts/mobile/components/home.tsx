...
import { getDailyQuests, type TrackKey, useProgress } from '@/context/progress';
...
export function HomeScreen() {
  ...
  const { profile, totalXp, level, levelProgress, currentStreak, isComplete, trackCompleted } = useProgress();
  ...
  return (
    <ScreenShell>
      <View style={styles.pagePadding}>
        ...
        <View style={styles.sectionHeader}><Text style={styles.sectionTitle}>Today’s quests</Text><Text style={styles.sectionMeta}>{doneToday}/{homeTasks.length} COMPLETE</Text></View>
        <View style={styles.taskList}>{homeTasks.map((task, index) => <TaskRow key={task.id} {...task} trackColor={TRACKS[task.track].color} index={index} />)}</View>
...
const styles = StyleSheet.create({
  ...
  sectionHeader: { flexDirection: 'row', alignItems: 'baseline', justifyContent: 'space-between', marginTop: 29, marginBottom: 11 }, sectionTitle: { color: '#F6FBFF', fontFamily: nativeTheme.typography.sans.bold, fontSize: 18 },
