...
export function MagicRings({ variant = 'rings', size = 240, playing = true }: { variant?: 'rings' | 'orb'; size?: number; playing?: boolean }) {
  ...
  useEffect(() => {
    ...
    try { renderer = createRingRenderer(gl, undefined, variant); } catch { return; }
    ...
    const run = () => {
      ...
      interval = undefined;
      if (focused && !reduced && !document.hidden && playing) interval = setInterval(() => { time += 1 / 24; renderer.draw(time); }, 1000 / 24);
    };
  ...
  }, [focused, reduced, variant, playing]);
  return <div aria-hidden="true" style={variant === 'orb' ? { position: 'relative', width: size, height: size, alignSelf: 'center', pointerEvents: 'none' } : { position: 'absolute', top: -70, left: 0, right: 0, height: 600, pointerEvents: 'none', opacity: .45 }}>
    <div style={{ position: 'absolute', inset: 0, background: variant === 'orb' ? 'radial-gradient(circle, #534395 0%, #211d48 40%, transparent 52%)' : 'radial-gradient(ellipse at center, transparent 26%, #55d6ff0d 27%, transparent 28%, transparent 38%, #8d7cff0d 39%, transparent 40%)' }} />
