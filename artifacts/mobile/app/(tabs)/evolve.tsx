...
export default function EvolveRoute() {
  ...
  const start = (c: Challenge) => {
    if (done(c) || programmeAccess({ minimumLevel: 1, released: true, prerequisites: c.programme?.prerequisites ?? [] }, level, training) !== 'Available') return;
  ...
  return <ScreenShell><View style={s.page}>
    ...
    <LinearGradient colors={['#183F4B', '#171D36']} style={s.rank}>
      <View style={s.row}><Feather name="award" size={35} color="#6ADDD1" /><Text style={s.kicker}>RANK EARNED · LEVEL {level}</Text></View>
    ...
    <Text style={s.section}>Your available programmes</Text>
    ...
    <Pressable accessibilityRole="button" onPress={() => router.push('/saved-materials' as Href)} style={s.button}><Text style={s.buttonText}>Open saved materials & call history</Text></Pressable>
    <Pressable accessibilityRole="button" onPress={() => router.push('/exercise-videos' as Href)} style={s.button}><Text style={s.buttonText}>Watch exercise demonstrations</Text></Pressable>
    <Text style={s.section}>Higher-rank programmes</Text>
    <Text style={s.body}>Earning a rank does not skip programme prerequisites.</Text>
    {RANKS.filter(r => r.level > 1).map(rank => <View key={rank.level} accessibilityLabel={`${rank.name}, programme coming later`} style={s.locked}>
      <Feather name="lock" size={22} color="#768D9F" /><View style={{ flex: 1 }}><Text style={s.lockTitle}>{rank.name}</Text><Text style={s.body}>{level >= rank.level ? 'Rank earned' : `Rank requires level ${rank.level}`} · Complete the foundation first</Text></View><Text style={s.kicker}>{programmeAccess({ minimumLevel: rank.level, released: false, prerequisites: [] }, level, training)}</Text>
    </View>)}
