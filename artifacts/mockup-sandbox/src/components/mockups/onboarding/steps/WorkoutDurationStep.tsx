...
import { WorkoutDuration, DURATION_OPTIONS, OnboardingStepProps } from '../types';
import { cn } from '@/lib/utils';

export function WorkoutDurationStep({ data, onNext, onBack }: OnboardingStepProps) {
  const selectedDuration = data.workoutDuration;

  const handleSelect = (duration: WorkoutDuration) => {
    onNext({ ...data, workoutDuration: duration });
  };
      ...
      >
        <div className="grid grid-cols-2 gap-4 max-w-xl mx-auto">
          {DURATION_OPTIONS.map((duration, index) => (
            ...
            >
              <motion.div
                animate={{ scale: selectedDuration === duration.value ? 1.1 : 1 }}
              ...
              <span className="text-xs text-muted-foreground">{duration.minutes} min sessions</span>
              {selectedDuration === duration.value && (
                <motion.div
